    ...
    <div className="min-h-screen" dir="rtl" style={{ backgroundColor: "hsl(30, 30%, 97%)", fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-border">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <h1 className="text-3xl font-bold" style={{ fontFamily: "'Satisfy', cursive", color: "hsl(15, 80%, 50%)" }}>
        ...
        <div className="relative z-10 h-full flex items-center justify-center text-center">
          <div className="text-white">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
              <Button
                className="rounded-full px-8 py-6 text-sm tracking-wider"
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <h3 className="text-4xl font-bold mb-2" style={{ fontFamily: "'Satisfy', cursive", color: "hsl(15, 80%, 50%)" }}>
            قائمة الطعام
          </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {products.map((product, i) => (
              ...
              >
                ...
                <div className="p-5">
                  <h4 className="font-semibold text-foreground mt-1 mb-1">{product.name}</h4>
                  {product.description && <p className="text-xs text-muted-foreground mb-2">{product.description}</p>}
                  <p className="font-bold text-lg" style={{ color: "hsl(15, 80%, 50%)" }}>{product.price} ج.م</p>
                </div>
              </motion.div>
            ))}
          </div>

...
const KidsToysStore = () => {
  ...
  const renderProductCard = (p: any, i: number) => (
    <motion.div key={p.id} initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="group bg-white rounded-[1.5rem] overflow-hidden shadow-sm cursor-pointer" style={{ border: "2px solid hsl(45,40%,90%)" }}>
      <div className="aspect-[4/5] overflow-hidden relative" style={{ backgroundColor: "hsl(45,50%,95%)" }}>
        {p.image_url ? <img src={p.image_url} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center"><ShoppingBag className="w-10 h-10 opacity-20" /></div>}
      ...
      <div className="p-3">
        <h4 className="font-bold text-sm mb-1" style={{ color: "hsl(280,40%,25%)" }}>{p.name}</h4>
        <p className="font-black text-sm" style={{ color: "hsl(280,60%,55%)" }}>{p.price} ج.م</p>
      </div>
    </motion.div>
  );
    ...
    <div className="min-h-screen" dir="rtl" style={{ backgroundColor: "hsl(45,50%,97%)" }}>
        ...
        groupedProducts.map((group, gi) => (
          <section key={group.category.id} className="max-w-7xl mx-auto px-6 py-12">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gi * 0.1 }} className="flex items-center gap-3 mb-8">
              <h3 className="text-3xl font-black" style={{ color: "hsl(280,60%,40%)" }}>{group.category.name}</h3>
              <span className="text-sm" style={{ color: "hsl(280,20%,55%)" }}>({group.products.length})</span>
            </motion.div>
      ...
      {uncategorizedProducts.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 py-12">
          <h3 className="text-3xl font-black mb-8" style={{ color: "hsl(280,60%,40%)" }}>أخرى</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">

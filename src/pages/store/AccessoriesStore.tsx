  ...
  return (
    <div className="min-h-screen text-white" dir="rtl" style={{ backgroundColor: "hsl(30, 15%, 6%)", fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {categories.map((cat, i) => (
              <motion.button
                ...
                className="px-6 py-3 rounded-full border border-white/20 text-white/80 hover:bg-white/10 transition-colors text-sm font-medium"
              >
                {cat}
              </motion.button>
            ))}
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        <h3 className="text-3xl font-bold mb-10" style={{ fontFamily: "'Cormorant Garamond', serif", color: "hsl(43, 74%, 49%)" }}>
          المنتجات المميزة
        </h3>
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product, i) => (
              ...
              >
                <div className="aspect-square overflow-hidden rounded-sm mb-4 border border-white/10">
                  {product.image_url ? (
                    <img
